...
public sealed class OpeningPublicationManagementManager : IDisposable
{
    ...
    private readonly ConcurrentDictionary<Guid, OpeningPublicationManagementSnapshot> snapshots = new();
    private readonly ConcurrentDictionary<Guid, SemaphoreSlim> gates = new();

    public OpeningPublicationManagementManager(
    ...
    public OpeningPublicationManagementSnapshot GetSnapshot(VenueConnectionConfiguration venue) =>
        snapshots.TryGetValue(venue.ProfileId, out var snapshot)
            ? snapshot
            : OpeningPublicationManagementSnapshot.NotLoaded;
    ...
    public bool ShouldLoad(VenueConnectionConfiguration venue)
    {
        ...
        if (snapshot.Status == OpeningPublicationManagementStatus.Loading) return false;
        if (snapshot.Status == OpeningPublicationManagementStatus.NotLoaded) return true;
    ...
    public Task<ApiResult<OpeningPublicationContextResponse>> LoadAsync(
        ...
        WithGateAsync(venue, async () =>
        {
            if (!force && !ShouldLoad(venue))
            {
                ...
                return existing.View is not null
                    ? ApiResult<OpeningPublicationContextResponse>.Succeeded(existing.View)
                    : ApiResult<OpeningPublicationContextResponse>.Failed(new ApiFailure(
                        ApiFailureKind.Unknown,
                        "OPENING_PUBLICATION_DATA_NOT_AVAILABLE",
                        existing.Message));
            }

            var attemptAt = DateTimeOffset.UtcNow;
            var existingSnapshot = GetSnapshot(venue);
            snapshots[venue.ProfileId] = existingSnapshot with
            {
                Status = OpeningPublicationManagementStatus.Loading,
                Message = existingSnapshot.View is null
                    ? "Loading opening publications..."
                    : "Refreshing opening publications...",
            ...
            if (!context.Success)
            {
                ...
                return ApiResult<OpeningPublicationContextResponse>.Failed(context.Failure!);
            }

            var result = await apiClient.GetOpeningPublicationsAsync(
            ...
            snapshots[venue.ProfileId] = new OpeningPublicationManagementSnapshot(
                OpeningPublicationManagementStatus.Ready,
                "Opening publications loaded.",
                result.Value,
                attemptAt,
                receivedAt);
            return result;
    ...
    public Task<ApiResult<SaveOpeningPublicationTemplateResponse>> SaveTemplateAsync(
        VenueConnectionConfiguration venue,
        ...
        WithAuthorizedMutationAsync(
            ...
            async context =>
            {
                var result = await apiClient.SaveOpeningPublicationTemplateAsync(
                    context.BaseUri!, context.AccessToken!, publicationCode, request, cancellationToken);
                if (result.Success) await RefreshCoreAsync(venue, context, cancellationToken);
                return result;
            },
            ApiResult<SaveOpeningPublicationTemplateResponse>.Failed,
            cancellationToken);
    ...
    public Task<ApiResult<GenerateOpeningPublicationsResponse>> GenerateAsync(
        ...
        WithAuthorizedMutationAsync(
            ...
            ApiResult<GenerateOpeningPublicationsResponse>.Failed,
            cancellationToken);

    public Task<ApiResult<SaveOpeningPublicationTextResponse>> SaveTextAsync(
        VenueConnectionConfiguration venue,
        long openingId,
        string publicationCode,
        ...
        WithAuthorizedMutationAsync(
            ...
            async context =>
            {
                var result = await apiClient.SaveOpeningPublicationTextAsync(
                    context.BaseUri!, context.AccessToken!, openingId, publicationCode, request, cancellationToken);
                if (result.Success) await RefreshCoreAsync(venue, context, cancellationToken);
                return result;
            },
    ...
    public void Clear(string message = "Opening publication data was cleared.")
    {
        foreach (var pair in snapshots)
            snapshots[pair.Key] = OpeningPublicationManagementSnapshot.NotLoaded with { Message = message };
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        log.Warning(
            "Opening-publication mutation succeeded but refresh failed: {Code} {Message}",
            result.Failure?.Code,
        ...
        snapshots[venue.ProfileId] = existing with
        {
            ...
            Message = "Opening publications changed, but the refreshed state could not be loaded.",
            LastAttemptAt = now
        };
    }
